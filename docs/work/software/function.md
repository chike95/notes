...
```js
function crc16(data) // CRC16校验输出Buffer
{
	var crc = 0xFFFF;
...
var crc16_tab_h = [ 0, -63, -127, 64, 1, -64, Byte.MIN_VALUE, 65, 1, -64, Byte.MIN_VALUE, 65, 0, -63,
	...
	Byte.MIN_VALUE, 65, 0, -63, -127, 64 ]; // crc16_tab高位
var crc16_tab_l = [ 0, -64, -63, 1, -61, 3, 2, -62, -58, 6, 7, -57, 5, -59, -60, 4, -52, 12, 13, -51, 15,
	...
	-122, -126, 66, 67, -125, 65, -127, Byte.MIN_VALUE, 64 ]; // crc16_tab低位
function calcCrc16_checksum(data, offset, len, preval) // crc16计算函数 输出16位无符号整数
{
	var ucCRCHi = (preval & 0xFF00) >> 8;
	var ucCRCLo = preval & 0xFF;
	for (var i = 0; i < len; i++) {
		var iIndex = (ucCRCLo ^ data[(offset + i)]) & 0xFF;
